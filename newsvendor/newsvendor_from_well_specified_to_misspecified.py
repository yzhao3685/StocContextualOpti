...
##    erm_std = np.std(erm_arr,axis=1);pto_std = np.std(pto_arr,axis=1);integ_std = np.std(integ_arr,axis=1)
    title = 'gamma = ' + str(gamma_specification) #'unconstrained case'    
    ...
    #pyplot.show()
    name = 'gamma_' + str(gamma_specification) #'true_'+data_dist+'_model_'+model_dist
    pyplot.savefig('error_bar_'+name+'.png', bbox_inches='tight')    
...
gamma_specification = 1 # choose 0.6, 0.7, 0.8, 0.9, 1. if gamma = 1, then model is correct
gaus_std = gamma_specification * np.ones(n_products) + (1 - gamma_specification) * np.array([5,4,3,2,1])*3
