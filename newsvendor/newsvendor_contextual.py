...
def generate_data(train_size,test_size,dist='gaussian'):
    if dist=='gaussian':
        x_train=np.random.uniform(0,1,size=(train_size,2))
        mean_arr=2+(x_train[:,0]+x_train[:,1])/2
        d_train=np.random.normal(mean_arr,np.ones(train_size))

        x_test=np.random.uniform(0,1,size=(test_size,2))
...
def get_regret(train_size,test_size,data_dist='uniform',model_dist='gaussian',over_param=0):
    x_train,d_train,x_test,d_test,w_oracle = generate_data(train_size,test_size,dist=data_dist)
...
def get_regret_arr(data_dist,model_dist,n_seed=5,over_param=0):
    ...
    pto_over_arr = np.zeros((len(n_list),n_seed))
    for i in range(len(n_list)):
        n=n_list[i]
        for j in range(n_seed):
            erm_regret, pto_regret,integ_regret,pto_over_param_regret = get_regret(n,100000,data_dist,model_dist,over_param)
            erm_arr [i,j]+=erm_regret
            pto_arr[i,j] +=pto_regret
            integ_arr[i,j]+=integ_regret
            if over_param==1:
                pto_over_arr[i,j]+= pto_over_param_regret
            if j%5==0:
                print('seed: ',j,'done')
        print('n: ',n,'regret ','ERM: ',round(erm_regret,5),'PTO: ',round(pto_regret,5),
              'PTO over param: ',round(pto_over_param_regret,5),'integrated',round(integ_regret,5))
...
def plot_error_bar(erm_arr,pto_arr,integ_arr):
    ...
    # title='true dist: '+data_dist+' model dist: '+model_dist
    title = specification # 'feature based case'
...
label_font_size=20
tick_font_size=15
n_list=[100,200,300,400,500] #[200
specification = 'Correctly specified' # 'Misspecified', 'Correctly specified'
...
n_seed = 50 #50 
erm_arr,pto_arr,integ_arr,pto_over_arr = get_regret_arr(data_dist,model_dist, n_seed,over_param=0)
